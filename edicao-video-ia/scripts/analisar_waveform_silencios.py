...
def main() -> None:
    ...
    if args.sample_rate <= 0 or args.window_ms <= 0 or args.min_silence_ms < 0:
        parser.error("Sample rate e janela devem ser positivos; silencio minimo nao pode ser negativo.")
    if not math.isfinite(args.threshold_db):
        parser.error("Threshold deve ser finito.")
    if any(value is not None and (not math.isfinite(value) or value < 0) for value in (start, end)):
        parser.error("Tempos devem ser finitos e nao negativos.")
    if end is not None and end <= (start or 0.0):
        parser.error("Fim deve ser maior que inicio.")
    offset = start or 0.0
